...
G4bool ThomasImelTail=true, OutElectrons=true;
...
G4double UnivScreenFunc ( G4double E, G4double Z, G4double A );	
	// makes scint yield only a fraction of the actual value
	// This is necessary to make tracking possible
...
G4S1Light::G4S1Light(const G4String& processName,
      ...
      : G4VRestDiscreteProcess(processName, type),
        SinglePhase(true),// turns tracking of thermal electrons on (false) or off (true) respectively
...
// this is the most important function, where all light & charge yields happen!
{
	...
	default: nDensity /= 131.293;
	  ScintillationYield = YieldReductionFactor*(48.814+0.80877*Density+2.6721*pow(Density,2.)); //REDUCED YIELD TO 1 PERCENT!!
	  ...
	  if ( Phase == kStateLiquid ) {
	    ...
	    if(!fAlpha) ExcitationRatio=0.07; //Nygren NIM A 603 (2009) p. 340
	      ...
	      ScintillationYield = YieldReductionFactor / (12.98*eV); } //Saito 2003 REDUCED YIELD TO 1 PERCENT!!
	...
	if(TotalEnergyDeposit < 1/(ScintillationYield) || 
		NumQuanta < 0)
	  NumQuanta = 0;
